/* ==========================================================================
   Lodestar Atlas — shared behaviour
   Vanilla JS, no dependencies, no build step.

   Contents
     1.  Utilities
     2.  Image fallback (never ship a broken image)
     3.  Header: transparent -> blurred night bar on scroll
     4.  Mobile navigation + dropdowns
     5.  Scroll reveals + slow media settle
     6.  Back to top
     7.  Accordions (FAQ)
...
(function () {
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      var seed = (el.dataset.fallback || el.alt || "lodestar-travel")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .slice(0, 40);
  ...
  /* ---------------------------------------------------------------------
     ...
     --------------------------------------------------------------------- */
  var CONSENT_KEY = "lodestar_consent_v1";
  ...
  function applyConsent(value) {
    ...
    window.lodestarConsent = value;
    document.dispatchEvent(new CustomEvent("lodestar:consent", { detail: value }));
  }
  ...
  function validateField(field) {
    ...
    if (value && input.type === "tel" && !PHONE_RE.test(value)) {
      setError(field, "Enter a US phone number, for example +1 (206) 555-0172.");
      return false;
    }
  ...
  if (plannerRoot) {
    ...
    function persist() {
      ...
      if (!consent) return;
      try { localStorage.setItem("lodestar_plan_v1", JSON.stringify(state)); }
    ...
    function restore() {
      ...
      try {
        var saved = JSON.parse(localStorage.getItem("lodestar_plan_v1") || "null");
